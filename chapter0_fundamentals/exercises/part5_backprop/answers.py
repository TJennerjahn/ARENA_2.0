...
def unbroadcast(broadcasted: Arr, original: Arr) -> Arr:
    ...
    '''
    diff_len = len(broadcasted.shape) - len(original.shape)
    broadcasted = broadcasted.sum(axis=tuple(range(diff_len)), keepdims=False)

    sum_dims = tuple([i for i, dim in enumerate(original.shape) if dim != broadcasted.shape[i]])
    broadcasted = broadcasted.sum(axis=sum_dims, keepdims=True)
    return broadcasted
...
class BackwardFuncLookup:
    def __init__(self) -> None:
        self._back_funcs: defaultdict[Callable, dict[int, Callable]] = defaultdict(dict)
        pass
...
def multiply_forward(a: Union[Tensor, int], b: Union[Tensor, int]) -> Tensor:
    ...
    if isinstance(a, Tensor) and isinstance(b, Tensor):
        ...
        out = tensor(array=np.multiply(a.array, b.array), requires_grad=requires_grad)
        if requires_grad:
        ...
        out = tensor(array=np.multiply(a.array, b), requires_grad=requires_grad)
        if requires_grad:
        ...
        out = tensor(array=np.multiply(a, b.array), requires_grad=requires_grad)
        if requires_grad:
...
def _expand(x: Arr, new_shape) -> Arr:
    ...
    '''
    new_shape = [d if d != -1 else x.shape[i] if i < len(x.shape) else x.shape[-1] for i,d in enumerate(new_shape)]
