...
class RewardAveraging(Agent):
    def __init__(self, num_arms: int, seed: int, epsilon: float, optimism: float):
        ...
        self.Q = np.zeros(num_arms, dtype=np.float32)
    ...
    def observe(self, action, reward, info):
        self.N[action] += 1
        self.Q[action] = ((self.N[action] - 1) * self.Q[action] + reward) / self.N[
            action
        ]

    def reset(self, seed: int):
        self.rng = np.random.default_rng(seed=seed)
        self.Q = np.full(num_arms, self.optimism, dtype=np.float32)
        self.N = np.zeros(num_arms)
...
class UCBActionSelection(Agent):
    ...
    def get_action(self):
        return np.argmax(
            self.Q + self.c * np.sqrt(np.log(self.t) / (self.N / self.epsilon))
        )
